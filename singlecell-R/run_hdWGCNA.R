...
module_enrich(
  ...
  cor_threshold   = 0.4          # 只保留 |kME| ≥ 0.6 的 hub genes
)
#####################################################
df_filtered <- subset(modules, module %in% c("Normal-human-FB-M16", "Normal-human-FB-M17"))
module_enrich(
  modules_df      = df_filtered,
  result_path     = paste0(resultpath,"enrich/"),
  dataset         = dataset,
  spefic_celltype = spefic_celltype,
  organism        = "human",     # 或 "human"
