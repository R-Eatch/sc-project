...
dataset2='S-AG'
dataset = 'S-MAG'
path1=f"../{dataset1}/2.filter/{dataset1}_noh_cleaned.h5ad"
path2=f"../{dataset2}/2.filter/{dataset2}_noh_cleaned.h5ad"
vars_use = ['sample']
random.seed(12345)
np.random.seed(12345)
Featuregenes = [    'ESR1', 'EPCAM', 'TOP2A', 'ACTA2', 'LALBA', 'LEF1', 'KRT23', 'ELF5', 'TCF4', 'KRT1', 'PRL-R', 'AR', 'PIGR','KRT8','KRT18-1','KRT14','KRT5',
...
def run_harmony(adata,vars_use=vars_use):
    ...
    return adata
def update_cell_annotations(adata, new_cell_type_dict, update_cell_type):
    """
    ...
    adata.obs['newcelltype'] = adata.obs['leiden'].map(cluster_to_celltype)
...
sc.tl.pca(adata_combined,use_highly_variable=True,random_state=random_state)
...
# In[29]:
update_cell_annotations(adata_combined, new_cell_type_dict, update_cell_type)

sc.pl.umap(
        adata_combined,
        color=['sample','leiden','gland','stage'],
...
if doFeatureplot:
    sc.pl.umap(
        adata_combined,
        color=[*Featuregenes,'sample','leiden','gland'],
        # increase horizontal space between panels
        wspace=0.5,
        size=3,
    ncols=3,
