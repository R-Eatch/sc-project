...
# 输入和输出路径示例
dataset='R-AG'
input_h5ad_path = "/data01/sunxuebo/project/scrnaseq/h5ad2seurat/processed/allsc_rabbit_v9_scrublet.h5ad"  # <-- 修改成自己的h5ad路径
...
Featuregenes = [
"EPCAM",'PRLR','CSN2','CSN3','KRT7','KRT15','KRT16','KRT23',"GPCPD1","LCN2","APOD","CLDN1","CDH1","XDH"
]
...
def run_preprocess(adata, top_genes=2000):
    ...
    """
    # 在对数转换前保存原始counts
    # 如果 adata.X 不是 raw counts（例如已是 log1p），需要根据实际情况处理
