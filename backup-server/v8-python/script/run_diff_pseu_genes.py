...
TOP_N: int = 10          # number of top genes kept *per stage*
...
def _select_top_by_stage(df: pd.DataFrame,
    ...
    for st, sub in df.groupby(stage_col):
        sig = sub[(sub["pvals_adj"] < pval_cutoff) & (sub["logfoldchanges"] > lfc_cutoff)]
        top_genes = (
...
# 过滤空值并转换为字符串，构成新基因集合
novel_genes = set(novel_df["Novel_Gene"].dropna().astype(str).tolist())
