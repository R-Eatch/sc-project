...
dataset <- 'R-MAG unique'
# --- 2. 读取数据 ---
go_file <- paste0("D:/111/GO_KEGG_Results/",dir_name,"/R.MG...R.AG_GO_Result.csv")
kegg_file <- paste0("D:/111/GO_KEGG_Results/",dir_name,"/R.MG...R.AG_KEGG_Result.csv")
...
kegg_ids_to_select <- c(
  ...
  'mmu05020',    # Prion disease (barrier/immune emphasis)
...
p_base <- ggplot() +
  ...
  # 标题
  labs(
    title = paste0("GO/KEGG Enrichment Results for Common HVGs In",dataset)
  ) +
  # 居中轴线
...
print(p_unlabeled)
# 保存无标签图
ggsave(paste0("GO_KEGG_common_NoLabels",dataset,".pdf"), plot = p_unlabeled, width = 7.5, height = 7, device = cairo_pdf)
ggsave(paste0("GO_KEGG_common_NoLabels",dataset,".png"), plot = p_unlabeled, width = 7.5, height = 7, dpi = 300, units = "in", bg = "white")

# 显示有标签图
print(p_labeled)
# 保存有标签图 (需要更宽的尺寸)
ggsave(paste0("GO_KEGG_common_SideLabels",dataset,".png"), plot = p_labeled, width = 9.5, height = 7, device = cairo_pdf) # 增加宽度
ggsave(paste0("GO_KEGG_common_SideLabels",dataset,".png"), plot = p_labeled, width = 9.5, height = 7, dpi = 300, units = "in", bg = "white") # 增加宽度