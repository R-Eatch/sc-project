...
spefic_celltype <- 'Epi-Krt7'
resultpath <- 'D:/Data/hdwgcna-result/'
RDSpath <- 'D:/Data/S-MG_for_cellchat.rds'
dataset <- 'S-MG'
n_genes <- 5
nThreads <- 8
##############################################
...
plot_module_native <- function(obj,
  ...
  ggsave(file_fp,  p_feature, width = 15, height = 2 + 2.5 * length(fp_list), dpi = 300)
  ggsave(file_dot, p_dot,     width = 10, height = 6,                dpi = 300)
...
seurat_obj <- MetacellsByGroups(
  ...
  group.by = c(groupby), # specify the columns in seurat_obj@meta.data to group by
  reduction = reduction, # select the dimensionality reduction to perform KNN on
  k = 25, # nearest-neighbors parameter
  max_shared = 10, # maximum number of shared cells between two metacells
  ident.group = 'newcelltype' # set the Idents of the metacell seurat object
)
...
seurat_obj <- ConstructNetwork(
  seurat_obj,
  tom_name = spefic_celltype,overwrite_tom = TRUE 
)
...
ggsave(filename =paste0(resultpath,dataset,'-',spefic_celltype,'hMEs-fp.png') ,plot = p3,dpi = 300,bg = 'white',width = 30,height = 15)
