...
for (dataset in datasetlist){
  ...
  for (group_name in groups) {
    ...
    go1 <- group_data %>% filter(logfoldchange > 0) %>% arrange(pvalue) %>% head(n_gene)

    #go1 <- go1 %>% filter(pvalue < 0.01)#之前的数据已经进行过筛选了
  ...
  if (output_go){
    ...
    ggsave(paste0(dataset,"_GO_Dotplot.pdf"),path = "D:/111/DEG-result",plot=p3,height = 30,width = 30,limitsize = FALSE)
  }
  if (output_kegg){
    ...
    ggsave(paste0(dataset,"_KEGG_Dotplot.pdf"),path = "D:/111/DEG-result",plot=p3,height = 30,width = 30,limitsize = FALSE)
  }
}
