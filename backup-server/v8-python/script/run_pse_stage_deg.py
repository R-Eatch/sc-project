...
lfc_cutoff  = 0.25
if_subcelltype = True
#subcelltype=["MaSC","MaSC-t2-sg","LumSEC-Lac","LumSEC-Lip","Epi-Krt7"]
#subcelltype=["ASC-sg",'Epi-Lgals7','Epi-Krt7','LumSEC-AG-Pip']
subcelltype=[
                    "MaSC",
                    "MaSC-Pro",
                    "LumSEC-Lac",
...
for ds in datasetlist:
    ...
    plt.tight_layout()
    fig.savefig(f"{ds}_pse_stage_heatmap.png", dpi=300)
    fig.savefig(f"{ds}_pse_stage_heatmap.pdf", dpi=300)
    plt.close(fig)
    df1 = export_deg_result(adata=adata)
    df1.to_csv(f'{dataset}_pseudotime_genes.csv', index=False)
...
fig1.savefig(f'{dataset}-pseudotime-umap.png', dpi=300, bbox_inches="tight")
fig1.savefig(f'{dataset}-pseudotime-umap.pdf', dpi=300, bbox_inches="tight")
...
sc.pl.umap(
    ...
    title=f'{dataset}UMAP colored by pse_stage',save=f'{dataset}_pse-stage_umap.png'
)
...
sc.pl.umap(
    ...
    size=20,
    legend_loc='right margin',
    title=f'{dataset}UMAP colored by pse_stage',save=f'{dataset}_pse-all_umap.png'
)
