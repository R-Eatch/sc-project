...
gland_column = 'gland'
n_top_genes=2000
path1=f"/data01/sunxuebo/project/scrnaseq/v8-python/S-MG/1.subset/S-MG-sub-newcelltype/S-MG-sub-newcelltype_cleaned.h5ad"
path2=f"/data01/sunxuebo/project/scrnaseq/v8-python/S-AG/1.subset/S-AG-sub-newcelltype/S-AG-sub-newcelltype_cleaned.h5ad"
vmin, vmax = 0.5,0.9
# In[4]:
...
adata1.obs['clusters']=adata1.obs['sub-newcelltype']
...
adata.obs['clusters'] = (adata.obs['clusters'].astype(str) + '-' +  adata.obs[gland_column].astype(str))
...
plt.tight_layout()
plt.savefig(f'{dataset}-sub_correlation_heatmap.png', dpi=300)
plt.show()
# ===== 3) 若需要同时保存结果DataFrame =====
correlation_df.to_csv(f"{dataset}-sub_correlation_results.csv", index=False)
...
plt.tight_layout()
plt.savefig(f'{dataset}-sub_correlation_dotplot.png', dpi=300)
plt.savefig(f'{dataset}-sub_correlation_dotplot.pdf', dpi=300)
plt.close()
