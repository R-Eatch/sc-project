...
sc.pl.umap(
        adata_combined,
        color=["newcelltype", "celltype", "stage",'sample','leiden','species'],
...
if doFeatureplot:
    sc.pl.umap(
        adata_combined,
        color=[*Featuregenes,'newcelltype','leiden','species'],
        # increase horizontal space between panels
        wspace=0.5,
        size=3,
    ncols=3,
...
#### Different gene test###
sc.tl.rank_genes_groups(adata_combined,groupby='species',method = 'wilcoxon')
sc.pl.rank_genes_groups_dotplot(adata_combined,groupby='species',n_genes=8,save=f'{dataset}_dotplot.png',min_logfoldchange=0.1)
