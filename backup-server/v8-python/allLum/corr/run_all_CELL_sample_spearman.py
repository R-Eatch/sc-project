...
datasets = ['M-MG','R-MG','S-MG','R-AG','S-AG','R-CG','M-SG','S-SG']
use_hvg=True
n_top_genes=3000
adata_list=[]
################
...
# In[ ]:
path = "/data01/sunxuebo/project/scrnaseq/h5ad2seurat/processed/v9_all_log1p.h5ad"

adata_combined=ad.read_h5ad(path)
print("read h5ad fINISH")
...
adata_combined.obs['celltype_dataset']=adata_combined.obs['sample']
...
# 存储结果
correlation_results = []
print("really begin celltype corr")
...
correlation_matrix.to_csv('ALL-correlation_matrix-sample.csv')
...
# 保存图像
plt.savefig('ALL-heatmap_correlation-sample.png', dpi=300, bbox_inches='tight')  # 保存图像
plt.show()  # 显示图像
...
plt.savefig('ALL-heatmap_correlation-cluster-sample.png', dpi=300, bbox_inches='tight')  # 保存图像
plt.show()  # 显示图像
