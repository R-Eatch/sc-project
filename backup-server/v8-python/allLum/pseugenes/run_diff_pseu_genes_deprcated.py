...
def _select_top_by_stage(df: pd.DataFrame,
    ...
    for st, sub in df.groupby(stage_col):
        sig = sub[(sub["pvals_adj"] < pval_cutoff) & (sub["logfoldchange"] > lfc_cutoff)]
        top_genes = (
            sig.sort_values("logfoldchange", ascending=False)[gene_col]
            .head(top_n)
            .tolist()
        )
...
# 定义文件路径和样本名称（CSV文件命名格式: "样本名_pseudotime_cluster.csv"）
file_paths = [
        "/data01/sunxuebo/project/scrnaseq/v8-python/R-MG/9.stage_DEG/R-MG_pseudotime_genes.csv",
        "/data01/sunxuebo/project/scrnaseq/v8-python/R-AG/9.stage_DEG/R-AG_pseudotime_genes.csv",
        "/data01/sunxuebo/project/scrnaseq/v8-python/S-MG/9.stage_DEG/S-MG_pseudotime_genes.csv",
        "/data01/sunxuebo/project/scrnaseq/v8-python/S-AG/9.stage_DEG/S-AG_pseudotime_genes.csv",
        "/data01/sunxuebo/project/scrnaseq/v8-python/M-MG/9.stage_DEG/M-MG_pseudotime_genes.csv",
        "/data01/sunxuebo/project/scrnaseq/v8-python/R-CG/9.stage_DEG/R-CG_pseudotime_genes.csv",
]
sample_names = ["R-MG", "R-AG", "S-MG", "S-AG", "M-MG", "R-CG"]
...
#         print(f"Loading sample: {sample_names[i]} from {file_path}...")
#         df = pd.read_csv(file_path)
#         gene_list = df.iloc[:, 0].tolist()  # 第一列作为基因列表
#         gene_dict[sample_names[i]] = set(gene_list)
#         print(f"Sample {sample_names[i]}: {len(gene_list)} genes found.\n")
#     return gene_dict
...
def _select_top_by_stage(df: pd.DataFrame,
    ...
    stage_top: Dict[str, List[str]] = {}
    for st, sub in df.groupby(stage_col):
        sig = sub[(sub["pval_adj"] < pval_cutoff) & (sub["logfoldchange"] > lfc_cutoff)]
        top_genes = (
            sig.sort_values("logfoldchange", ascending=False)[gene_col]
            .head(top_n)
            .tolist()
