...
# Assuming adata_combined and dataset are already defined in your environment
# dataset = "your_dataset_name"

# 1. Define variables to plot
keys = ["newcelltype", "celltype", "mergedcelltype", 'stage', 'sample', 'species']

# 2. Create Matplotlib Figure and Axes
# constrained_layout=True is the best way to automatically adjust subplots,
# legends, and titles to prevent them from overlapping.
fig, axes = plt.subplots(
    nrows=len(keys), 
    ncols=1, 
    figsize=(12, 5 * len(keys)), # Slightly increased width to accommodate legends
    constrained_layout=True 
)

# Handle the case if there is only one key (axes would not be a list)
if len(keys) == 1:
    axes = [axes]

# 3. Loop through variables to plot
for i, key in enumerate(keys):
    ax = axes[i]
    
    # Call scanpy plotting function
    # ax=ax: Plot onto the specific subplot we created
    # show=False: Prevent scanpy from showing/closing the plot immediately
    sc.pl.umap(
        adata_combined,
        color=key,
        ax=ax,
        ...
        show=False, 
        color_map='viridis',
        title=key      # Explicitly set title
    )
    
    # === Crucial Step: Rasterize Scatter Points ===
    # Get all collection objects in the current ax (for scatter plots, these are PathCollections)
    # Set their rasterized property to True.
    # This keeps text/axes as vectors (clear/editable) but makes dots pixels (small file size).
    for collection in ax.collections:
        collection.set_rasterized(True)

# 4. Save the image
# dpi=300: High resolution for the rasterized parts (the dots)
# constrained_layout handles the spacing, so we often don't need bbox_inches='tight',
# but using it is still safe for cutting off extra whitespace.
output_filename = f'{dataset}_merged_celltype_rasterized.pdf'
