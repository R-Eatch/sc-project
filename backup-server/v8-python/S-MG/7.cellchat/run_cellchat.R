...
dataset <- 'S-AG'
seuratObj <- readRDS(paste0("./", dataset, "_for_cellchat.rds"))
...
ggsave(paste0(dataset, "_Communication_BubblePlot.png"), plot = bubble_plot, width = 16, height = 12, dpi = 300)
print(bubble_plot)
...
#############################################
# 7.1 Contribution Analysis: Display the contribution of each cell group to the signaling communication
contribution_plot <- netAnalysis_contribution(cellchat, signaling = pathway_name)
contribution_plot <- contribution_plot + ggtitle(paste0(dataset, " - ", pathway_name, " Signaling Communication Contribution Analysis"))
ggsave(paste0(dataset, "_", pathway_name, "_Contribution.png"), plot = contribution_plot, width = 8, height = 6, dpi = 300)
