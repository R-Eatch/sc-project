...
dataset <- "S-SG"
cores <- 8
s.genes <- c("Prlr",'Krt23',"Epcam",'Acta2','Lalba','Elf5','Esr1','Lef1','Krt5')
brunch_gene <- s.genes
subset_celltype <- c("ASC",'LumSEC-AG-t1','LumSEC-AG-t2',"LumSEC-MG-like",'Lum-Fibro-like'
)
root_state <- 1 #for ordercells
subset_clusters <- c()
branch_point <- 2 ##for BEAM analysis
set.seed(2024)
subset_cells_number<- 5000
pse_gene_num <- 800
use_ordergene <- "~sample"
use_hvg=TRUE
...
if(use_raw){
  ...
  data <- GetAssayData(ea, assay = 'RNA', slot = 'data')
}
...
p3.6 <- plot_cell_trajectory(cds, color_by = "gland",cell_name_size=10)
...
p8 <- plot_genes_branched_heatmap(cds[top100,],
                            ...
                            show_rownames = T)
ggsave(paste0(dataset,"_point",branch_point,"_Branch_heatmap.png"), p8$ph_res, width = 6.5, height = 12)
ggsave(paste0(dataset,"_point",branch_point,"_Branch_heatmap.png"), p8$ph_res, width = 6.5, height = 12,dpi = 300)
