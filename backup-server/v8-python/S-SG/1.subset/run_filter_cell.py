...
dataset = 'S-EG'  # sample name, will affect input/output file names
h5adpath = f'S-SG_for_DEG.h5ad'  # path to read the h5ad file
...
Featuregenes = [
    ...
    'Aqp3','Shh','Atp1a1','Atp1b1','Procr','Wnt6','Krt5','Trp63','Lgr5','Krt5','Pip','Sox9','Sox10','Areg','Krt7','Krt23'
]
...
def do_umap_plots(adata, sample_prefix="sample", feature_genes=None, do_feature=False):
    ...
    sc.pl.umap(
        adata,
        color=["newcelltype", "subtype", "stage", "sample", "leiden", "gland"],
        wspace=0.5,
        size=3,
        ncols=3,
    ...
    size=3,
    ncols=3,
    save=f'{sample_prefix}_basic_umap.pdf'
    )
    if do_feature and feature_genes is not None:
        sc.pl.umap(
            adata,
            color=[*feature_genes,'subtype',"stage",'sample','leiden','gland'],
            wspace=0.5,
            size=3,
            ncols=4,
        ...
        size=3,
        ncols=4,
        save=f'{sample_prefix}_feature_umap.pdf'
        )
...
def do_deg(adata, sample_prefix="sample"):
    ...
    groupby='leiden',
    n_genes=5,
    save=f'{sample_prefix}_dotplot_leiden.pdf',
    min_logfoldchange=0.25
    )
...
def do_cell_barplot(adata, sample_prefix="sample"):
    ...
    fig.savefig(f'{sample_prefix}_cell_bar_plot.pdf')
    plt.show()
...
if __name__ == "__main__":
    ...
    # Optional: if you want to subset adata by cell type
    if subset_celltype and 'celltype' in adata_raw.obs:
        print("Filtering by specified cell types...")
        adata_raw = adata_raw[adata_raw.obs['celltype'].isin(celltypelist)].copy()
