...
datasetlist=['M-MG','R-MG','S-MG','R-AG','S-AG','R-CG','M-SG','S-SG']
processed_pairs=set()
catgory="stage"
epi='-Epi'
use_DEG=False
...
def do_samap(sp1,sp2,gd1,gd2,fn1,fn2):
    ...
    D.head()
    D.to_csv(f"./{catgory}/{sp1}{gd1}-{sp2}{gd2}-{catgory}{epi}-D.csv")
    
    MappingTable.head()
    MappingTable.to_csv(f"./{catgory}/{sp1}{gd1}-{sp2}{gd2}-{catgory}{epi}-MappingTable.csv")
    ...
    # Save the figure
    plt.savefig(f'./{catgory}/{sp1}{gd1}-{sp2}{gd2}-{catgory}{epi}-samap-heatmap.png', dpi=300, bbox_inches='tight')
...
for dataset1 in datasetlist:
    for dataset2 in datasetlist:  
        ...
        gd1=ls[1]
        if use_DEG:
            fn1 = f'./{dataset1}_counts_DEG_pr.h5ad'
        else:
        ...
        gd2=ls[1]
        if use_DEG:
            fn2 = f'./{dataset2}_counts_DEG_pr.h5ad'
        else:
