...
datasetlist=['M-MG','R-MG','S-MG','R-AG','R-CG','S-AG','M-SG','S-SG']
processed_pairs=set()
catgory="celltype"
...
def do_samap(sp1,sp2,gd1,gd2,fn1,fn2):
    ...
    D.head()
    D.to_csv(f"./{catgory}/{sp1}{gd1}-{sp2}{gd2}-{catgory}-D.csv")
    
    MappingTable.head()
    MappingTable.to_csv(f"./{catgory}/{sp1}{gd1}-{sp2}{gd2}-{catgory}-MappingTable.csv")
    ...
    # Save the figure
    plt.savefig(f'./{catgory}/{sp1}{gd1}-{sp2}{gd2}-{catgory}-samap-heatmap.png', dpi=300, bbox_inches='tight')
