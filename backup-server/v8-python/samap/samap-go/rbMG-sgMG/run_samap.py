...
fn1 = './R-MG_counts_pr.h5ad'
fn2 = './S-MG_counts_pr.h5ad'
sp1='rb'
sp2='sg'
gd1='MG'
gd2='MG'
...
keys = {sp1:'newcelltype',sp2:'newcelltype'}
D,MappingTable = get_mapping_scores(sm,keys,n_top = 0)
D.head()
D.to_csv(f"{sp1}{gd1}-{sp2}{gd2}-D.csv")
...
MappingTable.head()
MappingTable.to_csv(f"{sp1}{gd1}-{sp2}{gd2}-MappingTable.csv")
...
ax.set_title(f"{sp1}{gd1}-{sp2}{gd2} Cell Similarity Scores Based on SAMap", fontsize=16)
ax.set_xlabel("Cell Types", fontsize=12)
ax.set_ylabel("Cell Types", fontsize=12)

# Save the figure
plt.savefig(f'{sp1}{gd1}-{sp2}{gd2}-samap-heatmap.png', dpi=300, bbox_inches='tight')
...
sankey_plot(MappingTable, align_thr=0.1, species_order=[sp1,sp2],save_to=f'{sp1}{gd1}-{sp2}{gd2}-samap-heatmap.png',title=f'{sp1}{gd1}-{sp2}{gd2}-Cell Similarity')
