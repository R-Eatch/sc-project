...
# --- Global Variables ---
# 这是为了gland_ori.py进行修改的脚本，ori为原始umap坐标
glandlist = ['MG', 'SG', 'EG'] 
...
def run_plotly(adata, dataset):
    ...
    fig.write_html(f'./figure_velo/{dataset}-interactive_umap_plot_ori.html')
...
def process_velocity(dataset):
    ...
    file_path = f'./data/Final_{dataset}_merged_with_v9_UMAP.h5ad'
    ...
    if ax is not None: # 防止绘图失败
        ...
        fig.savefig(f'{scv.settings.figdir}/{dataset}_velo_ori.pdf', 
                    dpi=300, bbox_inches="tight", format="pdf")
        plt.close(fig)
    ax = scv.pl.velocity_embedding_stream(
        adata,
        basis="umap",
        color=color_key,          # categorical: celltype / cluster
        legend_loc="right margin",# 关键：图例放右侧边栏
        show=False,
        title=f"{dataset} Velocity",
    )
    if ax is not None: # 防止绘图失败
        fig = ax.get_figure()
        for artist in fig.findobj(match=matplotlib.artist.Artist):
            if hasattr(artist, "set_rasterized"):
                artist.set_rasterized(True)
        fig.savefig(f'{scv.settings.figdir}/{dataset}_velo_rightLABEL_ori.pdf',
                    dpi=300, bbox_inches="tight", format="pdf")
        plt.close(fig)
   ...
   # scv.pl.proportions(adata, groupby=color_key, save=f'{dataset}_proportions.png')
    ...
    # 10. 保存最终结果
    adata.write(f'./data/{dataset}_velofinished_ori.h5ad')
