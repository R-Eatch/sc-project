...
#### 全局变量 ####
DATASETS <- c("M-MG", "R-MG", "S-MG", "S-AG", "R-AG", "R-CG")
cores <- 1
DRAW_PLOTS <- TRUE
do_preprocess <-FALSE
# ---- Paths aligned with previous step ----
in_dir  <- file.path("result", "seurat_rds")
...
process_one_dataset <- function(dataset, cores = 1) {
  ...
  out_cds_rds <- file.path(out_dir, paste0(dataset, "_cds_for_subset_sec.rds"))
...
if(FALSE){
  cds_subset <- load_monocle_objects(directory_path=paste0('D:/Data/',dataset,'_cds_for_subset_sec.rds'))
  ...
  saveRDS(cds_subset, file=paste0('D:/Data/',dataset,'_subset_cds_sec.rds'))
}

process_one_dataset <- function(dataset) {

  in_cds_rds  <- file.path(in_dir, paste0(dataset, "_subset_cds_sec.rds"))
  ...
  ggsave(out_ct_pdf, p_ct, width = 8, height = 6)
  message("[SAVE] ", out_ct_pdf)
    ...
    ggsave(out_pt_pdf, p_pt, width = 8, height = 6)
