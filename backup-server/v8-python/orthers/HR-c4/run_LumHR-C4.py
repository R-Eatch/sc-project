...
for ax in fig.axes:
    for coll in ax.collections:
        if isinstance(coll, mc.PathCollection):   # 只栅格化散点
            coll.set_rasterized(True)
# 3) 保存为 PDF：dpi 只影响被 rasterize 的那部分（也就是点）
fig.savefig(f'{dataset}-umap-lumhr-c4-all.pdf', bbox_inches="tight", dpi=300)
plt.close(fig)

sc.tl.score_genes(adata_sub,gene_list=filtered_list,score_name='c4')

sc.pl.umap(adata_sub,color=[*filtered_list,'Pip','Krt7','Kit','Anxa1','Prlr','Pgr','Esr1','Pigr','c4','leiden'],ncols=3,show=False,cmap="coolwarm")
fig = plt.gcf()
for ax in fig.axes:
    for coll in ax.collections:
        if isinstance(coll, mc.PathCollection):   # 只栅格化散点
            coll.set_rasterized(True)
# 3) 保存为 PDF：dpi 只影响被 rasterize 的那部分（也就是点）
fig.savefig(f'{dataset}-umap-lumhr-c4-sub.pdf', bbox_inches="tight", dpi=300)
plt.close(fig)
