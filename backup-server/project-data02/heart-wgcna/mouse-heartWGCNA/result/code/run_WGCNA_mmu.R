...
#   - 导出每个模块 topN (|kME|) 的互作网络到 Cytoscape 文件（edges/nodes）
#
# UPDATE:
#   - 两个数据集使用独立参数：所有可调参数按 dataset 分开定义（全局变量写法不变）
#   - 修复 numericLabels=TRUE 时 kME/网络导出按数字标签列名 (kME_ME0/ME1/...) 选择
#   - 修复参数错位导致 moduleLabels 传入 list 的报错
# ============================================================
...
DATASETS <- list(
  heart = list(
    # Mouse HEART datExpr (samples x genes) produced by your mouse-heart preprocess script
    datExpr_csv = "D:/111/mouse-heartWGCNA/data/heart/heart_datExpr_samplesXgenes_VST.csv",
    outdir      = "D:/111/mouse-heartWGCNA/result/WGCNA_heart",
    
    # Trait definition (binary):
    #   negative (0) = sample name contains 'sham'
  ...
  lung = list(
    datExpr_csv = "D:/111/mouse-heartWGCNA/data/lung_pao/lung_pao_datExpr_samplesXgenes_VST.csv",
    outdir      = "D:/111/mouse-heartWGCNA/result/WGCNA_lung_pao",
    
    trait_name  = "POS",
    
    # negative (0) = Ctrl_M*  (PAO arm healthy controls)
    neg_regex   = "^Ctrl_M[0-9]+",
    
    # positive (1) = D*_PAO_M* (PAO infected, multiple timepoints)
    pos_regex   = "^D[0-9]+_PAO_M[0-9]+",
...
# -----------------------------
# Per-dataset parameter sets (EDIT ME)
# NOTE:
#   1) 在 list() 里必须用 “name = value”，不要用 “name <- value”。
#   2) 每个 dataset 的参数都放在 PARAMS$heart / PARAMS$lung 里。
# -----------------------------
PARAMS <- list(
  heart = list(
    # Basic thresholds
...
plot_soft_threshold <- function(sft, out_png, powers, target_R2) {
...
# NOTE: moduleLabels is net$colors (numeric labels per gene). Needed for numericLabels=TRUE.
export_kME_and_plots <- function(datExpr, MEs, traits, moduleColors, moduleLabels, outdir, trait_col) {
  ...
  lab_by_gene <- setNames(as.integer(moduleLabels), names(moduleColors))
  
  for (m in mods) {
    if (m == "grey") next
    
    labs <- unique(lab_by_gene[moduleColors == m])
    labs <- labs[!is.na(labs)]
    if (length(labs) == 0) next
    lab <- labs[1]
    
    # numericLabels=TRUE => kME columns are kME_ME0, kME_ME1, ...
    kcol <- paste0("kME_ME", lab)
...
export_module_networks <- function(datExpr, moduleColors, moduleLabels, geneInfo, outdir, softPower, networkType, TOMType,
                                   EXPORT_NETWORKS, NETWORK_TOP_N, NETWORK_TOM_THRESH) {
  ...
  for (m in mods) {
    ...
    if (length(labs) == 0) next
    if (length(labs) > 1) {
      msg("WARNING: color module %s maps to multiple numeric labels (%s); using the first.",
          m, paste(labs, collapse = ","))
    }
    lab <- labs[1]
    ...
    if (requireNamespace("igraph", quietly = TRUE)) {
      # Keep original behavior; only guard against rare read failures.
      try({
...
# -----------------------------
run_wgcna_one <- function(cfg, parset) {
  ...
  if (parset$DO_MEAN_FILTER) {
    n0 <- ncol(datExpr)
    datExpr <- mean_filter(datExpr, parset$MEAN_LOGCPM_MIN)
  ...
  if (parset$DO_TOP_VAR_GENES) {
    n0 <- ncol(datExpr)
    datExpr <- top_variable_genes(datExpr, parset$TOP_VAR_N)
  ...
  if (parset$DO_OUTLIER_CUT) {
    cutH <- parset$OUTLIER_CUT_HEIGHT
  ...
  sft <- pickSoftThreshold(datExpr, powerVector = parset$POWERS, networkType = parset$WGCNA_PARAMS$networkType, verbose = 5)
  plot_soft_threshold(sft, file.path(cfg$outdir, "02_soft_threshold.png"), parset$POWERS, parset$TARGET_SFT_R2)
  softPower <- choose_power(sft, target_R2 = parset$TARGET_SFT_R2)
  ...
  msg("Running blockwiseModules... (minModuleSize=%d, deepSplit=%d, mergeCutHeight=%.2f)",
      parset$MIN_MODULE_SIZE, parset$DEEP_SPLIT, parset$MERGE_CUT_HEIGHT)
  tomBase <- file.path(cfg$outdir, "TOM")
  
  net <- blockwiseModules(
    datExpr,
    power = softPower,
    networkType = parset$WGCNA_PARAMS$networkType,
    TOMType = parset$WGCNA_PARAMS$TOMType,
    minModuleSize = parset$MIN_MODULE_SIZE,
    reassignThreshold = parset$REASSIGN_THRESHOLD,
    mergeCutHeight = parset$MERGE_CUT_HEIGHT,
    deepSplit = parset$DEEP_SPLIT,
    pamRespectsDendro = parset$PAM_RESPECTS_DENDRO,
    maxBlockSize = parset$WGCNA_PARAMS$maxBlockSize,
    numericLabels = parset$WGCNA_PARAMS$numericLabels,
    saveTOMs = parset$WGCNA_PARAMS$saveTOMs,
  ...
  sig <- which(is.finite(cor_vec) & is.finite(p_vec) & (abs(cor_vec) >= parset$COR_THRESH) & (p_vec <= parset$P_THRESH))
  ...
  geneInfo <- export_kME_and_plots(datExpr, MEs, traits, moduleColors, labels, cfg$outdir, cfg$trait_name)
  export_module_networks(
    datExpr, moduleColors, labels, geneInfo, cfg$outdir, softPower,
    parset$WGCNA_PARAMS$networkType, parset$WGCNA_PARAMS$TOMType,
    parset$EXPORT_NETWORKS, parset$NETWORK_TOP_N, parset$NETWORK_TOM_THRESH
  )
  ...
  saveRDS(list(
    ...
    softPower     = softPower,
    params        = list(
      MIN_MODULE_SIZE = parset$MIN_MODULE_SIZE,
      DEEP_SPLIT = parset$DEEP_SPLIT,
      MERGE_CUT_HEIGHT = parset$MERGE_CUT_HEIGHT,
      PAM_RESPECTS_DENDRO = parset$PAM_RESPECTS_DENDRO,
      REASSIGN_THRESHOLD = parset$REASSIGN_THRESHOLD,
      networkType = parset$WGCNA_PARAMS$networkType,
      TOMType = parset$WGCNA_PARAMS$TOMType,
      DO_TOP_VAR_GENES = parset$DO_TOP_VAR_GENES,
      TOP_VAR_N = parset$TOP_VAR_N,
      DO_MEAN_FILTER = parset$DO_MEAN_FILTER,
      MEAN_LOGCPM_MIN = parset$MEAN_LOGCPM_MIN,
      POWERS = parset$POWERS,
      TARGET_SFT_R2 = parset$TARGET_SFT_R2,
      P_THRESH = parset$P_THRESH,
      COR_THRESH = parset$COR_THRESH,
      EXPORT_NETWORKS = parset$EXPORT_NETWORKS,
      NETWORK_TOP_N = parset$NETWORK_TOP_N,
      NETWORK_TOM_THRESH = parset$NETWORK_TOM_THRESH,
      DO_OUTLIER_CUT = parset$DO_OUTLIER_CUT,
      OUTLIER_CUT_HEIGHT = parset$OUTLIER_CUT_HEIGHT
    ),
...
for (nm in names(DATASETS)) {
  if (!nm %in% names(PARAMS)) stop("No parameter set found in PARAMS for dataset key: ", nm)
  nm='lung'
  run_wgcna_one(DATASETS[[nm]], PARAMS[[nm]])
}
msg("All done.")
""
