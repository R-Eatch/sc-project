...
#   2) Map Ensembl gene IDs -> gene_name using a GTF
#   3) Collapse duplicate gene_name rows by sum
#   4) Low-expression filtering using edgeR::filterByExpr (counts-level)
#   5) DESeq2 VST transform (blind=TRUE) for WGCNA input
#   6) Save:
...
# SETTINGS (EDIT ME)
# =========================
LUNG_COUNTS <- "D:/111/mouse-heartWGCNA/data/GSE233206_Tissue_RNAseq_rawCounts/GSE233206_Tissue_RNAseq_rawCounts.txt"
MOUSE_GTF   <- "D:\\111\\mouse-heartWGCNA\\data\\Mus_musculus.GRCm39.115\\Mus_musculus.GRCm39.115.gtf"

OUT_DIR <- "D:/111/mouse-heartWGCNA/data/lung"
PREFIX  <- "lung"
dir.create(OUT_DIR, showWarnings = FALSE, recursive = TRUE)

# Group inference for lung:
# samples whose names contain CONTROL_PATTERN are considered "control"; others are "case"
CONTROL_PATTERN <- "TC"   # change if needed
...
count_mat <- as.matrix(dt[, ..count_cols])
...
message("Sample columns preview:")
print(head(count_cols, 20))
