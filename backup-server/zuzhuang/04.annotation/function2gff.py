    ...
    with open(gff_file_name, 'r') as gff_file, open(output_file_name, 'w') as output_file:
        for line in gff_file:
            ...
            if gene_id in annotation:
                ...
                if feature_type == 'gene':  # 仅在gene行添加预测文本
                    attributes += ';prediction_text=' + prediction_text
            parts[-1] = attributes
            output_file.write('\t'.join(parts) + '\n')


def main():
    annotation_file = "C:\\Users\\eatch\\Desktop\\annotation_test.txt"  # 替换为你的注释文件路径
    gff_file = "C:\\Users\\eatch\\Desktop\\test_gff.gff"  # 替换为你的GFF文件路径
